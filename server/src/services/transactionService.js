...
export const allTransactionsData = async (page = 1, rowsPerPage = 10, searchQuery = '') => {
    let query = {};
    if (searchQuery) {
        query.$or = [
            { title: { $regex: searchQuery, $options: 'i' } },
            { description: { $regex: searchQuery, $options: 'i' } },
            { price: { $regex: String(searchQuery), $options: 'i' } }
        ];
    }
    ...
    try {
        ...
        .skip((page - 1) * rowsPerPage) // 1-based indexing
            .limit(rowsPerPage);
...
export const TransactionsStatisticsData = async (selectedMonth) => {
    try {
